        ...
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 bg-title-right">
            ...
            @if (session('impersonate'))
                <a class="btn b-all waves-effect waves-light pull-right" data-toggle="tooltip"
                    data-original-title="{{ __('messages.stopImpersonation') }}" data-placement="left"
            ...
            <div class="col-md-4 pull-right hidden-xs hidden-sm  m-r-10">
                ...
                <div class="btn-group dropdown keep-open pull-right m-l-10">
                    <button aria-expanded="true" data-toggle="dropdown"
                        class="btn b-all dropdown-toggle waves-effect waves-light" type="button"><i
                            class="icon-settings"></i>
                    </button>
                    <ul role="menu" class="dropdown-menu  dropdown-menu-right dashboard-settings">
                        <li class="b-b">
                            <h4>@lang('modules.dashboard.dashboardWidgets')</h4>
                        </li>
                        ...
                        <li>
                            <button type="button" id="save-form"
                                class="btn btn-success btn-sm btn-block">@lang('app.save')</button>
                        </li>
                ...
                <select class="selectpicker language-switcher  pull-right" data-width="fit">
                    ...
                    @foreach ($languageSettings as $language)
                        <option value="{{ $language->language_code }}" @if ($global->locale == $language->language_code) selected @endif
                            data-content='<span class="flag-icon @if ($language->language_code == 'zh-CN') flag-icon-cn @elseif($language->language_code == 'zh-TW') @elseif($language->language_code == 'ar') flag-icon-sa  @elseif($language->language_code == 'pt-br') flag-icon-br @else flag-icon-{{ $language->language_code }} @endif"></span>'>
            ...
            @if ($company->show_update_popup == 1 && $paymentCredential->stripe_status == 'active')
                ...
                <div class="col-md-12">
                    <div class="alert alert-info alert-dismissable">
                        <button type="button" class="close" data-dismiss="alert" onclick="hidePopUp()" aria-hidden="true">
                                    ...
                                    <div class="col-xs-9 text-right">
                                        ...
                                        <span class="counter">
                                            @if ($counts->totalEmployees > 0)
                                                {{ round(($counts->totalTodayAttendance / $counts->totalEmployees) * 100, 2) }}
                                            @else
                                                0
                                            @endif
                                        </span>%
                                        <span
                                            class="text-muted">({{ $counts->totalTodayAttendance . '/' . $counts->totalEmployees }})</span>
                                    </div>
                                </div>
                            </div>
                        </a>
                            ...
                            <div class="col-xs-12">
                                <div class="white-box">
                                    ...
                                    <h6 style="line-height: 2em;"><span
                                            class=" label label-danger">@lang('app.note'):</span> @lang('messages.earningChartNote') <a
                                            href="{{ route('admin.settings.index') }}"><i
                                                class="fa fa-arrow-right"></i></a></h6>
                                </div>
                            </div>
                                    ...
                                    <ul class="list-task list-group" data-role="tasklist">
                                        @forelse($newTickets as $key=>$newTicket)
                                            <li class="list-group-item" data-role="task">
                                                {{ $key + 1 }}. <a
                                                    href="{{ route('admin.tickets.edit', $newTicket->id) }}"
                                            ...
                                            @if ((!is_null($task->project_id) && !is_null($task->project)) || is_null($task->project_id))
                                                <li class="list-group-item row" data-role="task">
                                                    <div class="col-xs-9">
                                                        {!! $key +
                                                            1 .
                                                            '. <a href="javascript:;" data-task-id="' .
                                                            $task->id .
                                                            '" class="show-task-detail">' .
                                                    ...
                                                    <label
                                                        class="label label-danger pull-right col-xs-3">{{ $task->due_date->format($global->date_format) }}</label>
                                                </li>
                                            @endif
                                        @empty
                                            <li class="list-group-item" data-role="task">
                                            ...
                                            <li class="list-group-item row" data-role="task">
                                                <div class="col-xs-9">
                                                    ...
                                                    <a href="{{ route('admin.leads.show', $follows->id) }}"
                                                        class="text-danger">{{ ucwords($follows->company_name) }}</a>
                                    ...
                                    <div class="steamline">
                                        @forelse($projectActivities as $activ)
                                            <div class="sl-item">
                                                ...
                                                <div class="sl-right">
                                                    <div>
                                                        <h6><a href="{{ route('admin.projects.show', $activ->project_id) }}"
                                                                class="font-bold">{{ ucwords($activ->project->project_name) }}:</a>
                                                            {{ $activ->activity }}</h6> <span
                                ...
                                <div class="panel-body">
                                    <div class="steamline">
                                        @forelse($userActivities as $key=>$activity)
                                            <div class="sl-item">
                                                <div class="sl-left">
                                                    <img src="{{ $activity->user->image_url }}" alt="user"
